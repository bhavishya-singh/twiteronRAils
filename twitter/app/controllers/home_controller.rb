class HomeController < ApplicationController
  ...
  def follow
    ...
    follow_mapping = FollowMap.where(:follower_id => current_user.id, :followee_id => followee_id).first
    unless follow_mapping
        follow_mapping = FollowMap.create(:follower_id => current_user.id, :followee_id => followee_id)
    else
